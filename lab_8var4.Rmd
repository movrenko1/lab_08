...
tree.boston <- rpart( high.medv~ . -medv, Boston)
summary(tree.boston)
# график результата
...
tree.boston <- rpart(high.medv~ . -medv, Boston, subset = train)
# делаем прогноз
...
MSE на тестовой выборке равна 0.08.
...
```{r fourth}
set.seed(my.seed)
boost.boston <- gbm(high.medv ~ . -medv, data = Boston[train, ], 
